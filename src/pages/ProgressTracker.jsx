...
const ProgressTracker = () => {
  ...
  const chartData = {
    labels: groupedData.map(
      (result) => `${result.topic} (${result.attempts} attempts)`
    ),
    datasets: [
      {
        label: "Average Score",
  ...
  return (
    <div className="min-h-screen w-screen bg-gradient-to-r from-purple-800 via-black to-blue-900 text-white flex flex-col">
      <NavBar />
      <main className="flex flex-col items-center justify-center flex-grow p-5 sm:p-10">
        ...
        <div className="max-w-4xl w-full">
          <h2 className="text-xl sm:text-2xl font-bold mb-4">
            View Previous Records by Topic
          </h2>
          <select
            className="p-2 bg-white text-black rounded mb-4 w-full sm:w-auto"
            value={selectedTopic || ""}
