...
function QuizGenerator() {
  ...
  if (!isAuthenticated) {
    navigate("/login", {
      state: { message: "Please log in first before generating the quiz." },
    });
  ...
  return (
    <div className="min-h-screen w-screen bg-gradient-to-r from-purple-800 via-black to-blue-900 text-white flex flex-col">
      <NavBar />
      <motion.main
        className="flex flex-col items-center justify-center flex-grow mb-36 px-5 sm:px-10"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1 }}
      >
        <h2 className="text-4xl font-bold text-purple-300 mb-7 text-center">
          Quiz Generator
        </h2>
        ...
        {loading ? (
          ...
          >
            <div className="mb-4">
              <label className="block text-lg text-purple-300 mb-2">
                Select a Topic
              </label>
              <input
                type="text"
